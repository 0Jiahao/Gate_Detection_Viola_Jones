...
{ private:
  ...
  public:
    ...
    virtual void process(jevois::InputFrame && inframe, jevois::OutputFrame && outframe) override
    {
      ...
      // cv::inRange(img, cv::Scalar(20, 140, 40),
      //             cv::Scalar(255, 190, 140), bw);

      // // to output image
      // cv::cvtColor(bw,bw,CV_GRAY2BGR);
      // paste_fut.get(); inframe.done();
      // VJDetector.VJGateDetection(bw);
      ...
      // gray scale
      paste_fut.get(); inframe.done();
      VJDetector.VJGateDetection(img);
      
      // Wait for paste thread to complete and let camera know we are done processing the input image:
      jevois::rawimage::convertCvBGRtoRawImage(img,outimg,1);
